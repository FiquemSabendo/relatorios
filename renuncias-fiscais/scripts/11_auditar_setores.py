...
import duckdb
from classificacao_setorial import classificar_estabelecimentos,classificar_cnae,ESPECIFICAS,DIVISOES,VERSION,SETOR_POR_ATIVIDADE
...
correspondencias=[dict(cnae=k,descricao_origem=d,secao=s,atividade=classificar_cnae(k)[0],setor=SETOR_POR_ATIVIDADE[classificar_cnae(k)[0]],regra=classificar_cnae(k)[1],descricao_ibge=ibge.get(k,{}).get('descricao','Não localizado no catálogo consultado')) for k,d,s in classes]
save('dicionario_cnae_setor.csv',correspondencias)
save('regras.csv',[dict(prioridade=5-len(k),prefixo=k,atividade=v,setor=SETOR_POR_ATIVIDADE[v]) for rules in [ESPECIFICAS,DIVISOES] for k,v in sorted(rules.items())])
...
for r in rows:
 key=r['setor'];v=values[r['estab_id']];by[key]['cnpjs']+=1;by[key]['valor']+=v
...
for root,rs in roots.items():
 principal=max(rs,key=lambda r:(values[r['estab_id']],-r['estab_id']));top.append(dict(raiz=root,nome=principal['razao_social'],setor=principal['setor'],atividade=principal['atividade'],cnae=principal['cnae_original'],metodo=principal['regra'],n_setores_na_raiz=len({x['setor'] for x in rs}),valor=sum(values[r['estab_id']] for r in rs)))
